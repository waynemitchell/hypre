...
#ifdef OMP_UPDATE_ISSUE
     double bsumh=0.0,bsumd=0.0;
     
     double resultd = 0.0;
     double resulth = 0.0;

#pragma omp target teams  distribute  parallel for private(i) num_teams(100) thread_limit(32) reduction(+:bsumd)  map(tofrom:bsumd)
     for (i = 0; i < y_size; i++)
       bsumd += b_data[i] * b_data[i];
     
     for (i = 0; i < y_size; i++)
       bsumh += b_data[i] * b_data[i];
     
#pragma omp target teams  distribute  parallel for private(i) num_teams(100) thread_limit(32) reduction(+:resultd)  map(tofrom:resultd)
     for (i = 0; i < y_size; i++)
       resultd += y_data[i] * y_data[i];
     
     for (i = 0; i < y_size; i++)
       resulth += y_data[i] * y_data[i];
...
#pragma omp target teams  distribute  parallel for private(i) is_device_ptr(y_data,b_data)
     ...
     //for(i=0;i<y_size;i++) y_data[i] = b_data[i];
     UpdateDRC(y);
     //printRC(y,"AFTER SETTING Y++B");
 
 //#pragma omp target update from(y_data[0:y_size])
 //#pragma omp target update device(0) from(y->data[0:y->size])
 /* omp_target_memcpy(y->data,get_device_pointer(y->data), */
 /*       y->size*sizeof(double),0,0, */
...
#pragma omp target teams  distribute  parallel for private(i) num_teams(100) thread_limit(32) reduction(+:resultd)  map(tofrom:resultd)
     ...
     if (fabs((resulth-resultd)/resulth)>0.1) {
       printf("%p (%p) Y-DIFFER %g %g ( B = %g %g, ptr = %p )  size = %d \n",y_data,get_device_pointer(y_data),resulth,resultd,bsumh,bsumd,b_data,y_size);
#pragma omp target data use_device_ptr(y_data)
       printf("On the device the pointer is %p\n",y_data);
     }
...
//SyncVectorToHost(b);
//printf("Matvec with %d teams & %d threads for nrows = %d\n",num_teams,num_threads,num_rows);
...
#pragma omp target teams  distribute  parallel for num_teams(num_teams) thread_limit(num_threads) schedule(static,1) 
//#pragma omp target teams  distribute  parallel for schedule(static,1) if (0)
#endif
