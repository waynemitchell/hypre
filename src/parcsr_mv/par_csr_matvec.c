...
#pragma omp parallel for HYPRE_SMP_SCHEDULE
   ...
   */
   ...
   y_local->dev->ref_count=0;
#endif
#ifdef HYPRE_USE_CUDA
   PUSH_RANGE("PMV MV 1",0)
     y_local->dev->bring_from_device=0;
   HYPRE_Real fraction=diag->dev->fraction;
  if (hypre_CSRMatrixNumNonzeros(A)>CUDA_MATVEC_CUTOFF)
    hypre_CSRMatrixMatvecOutOfPlaceHybrid2Async( alpha, diag, x_local, beta, b_local, y_local, 0,fraction,0);
  else
...
#endif
#ifdef HYPRE_USE_CUDA
   y_local->dev->bring_from_device=1;
   POP_RANGE
#endif
#ifdef HYPRE_PROFILE
   hypre_profile_times[HYPRE_TIMER_ID_HALO_EXCHANGE] -= hypre_MPI_Wtime();
   ...
   if (num_cols_offd) {
     if (y_local->dev->ref_count==1){
       y_local->dev->send_to_device=0; // =0 means y_local will not be copied onto the device
       if (y_local->dev->nosync)
	 hypre_CSRMatrixMatvecOutOfPlaceHybrid2Async(alpha,offd,x_tmp,1.0,y_local,y_local,0,fraction,1); 
       else
	 hypre_CSRMatrixMatvecOutOfPlaceHybrid2(alpha,offd,x_tmp,1.0,y_local,y_local,0,fraction); 
...
hypre_ParCSRMatrixMatvec( HYPRE_Complex       alpha,
                          ...
                          hypre_ParVector    *y )
{
   return hypre_ParCSRMatrixMatvecOutOfPlaceBaseline(alpha, A, x, beta, y, y);
}
HYPRE_Int
hypre_ParCSRMatrixMatvecDevice( HYPRE_Complex       alpha,
                          hypre_ParCSRMatrix *A,
                          hypre_ParVector    *x,
                          HYPRE_Complex       beta,
                          hypre_ParVector    *y )
...
#ifdef HYPRE_USE_CUDA
HYPRE_Int
hypre_ParCSRMatrixMatvecSpec( HYPRE_Complex       alpha,
                          hypre_ParCSRMatrix *A,
                          hypre_ParVector    *x,
                          HYPRE_Complex       beta,
			      hypre_ParVector    *y , HYPRE_Real *l1_norms)
{
  return hypre_ParCSRMatrixMatvecOutOfPlaceSpec(alpha, A, x, beta, y, y,l1_norms);
}
HYPRE_Int
hypre_ParCSRMatrixMatvecOutOfPlaceSpec( HYPRE_Complex       alpha,
                                    hypre_ParCSRMatrix *A,
...
// y = alpha*A*x + beta*b
HYPRE_Int
hypre_ParCSRMatrixMatvecOutOfPlaceBaseline( HYPRE_Complex       alpha,
                                    hypre_ParCSRMatrix *A,
                                    hypre_ParVector    *x,
                                    HYPRE_Complex       beta,
                                    hypre_ParVector    *b,
