...
#pragma omp barrier
      ...
      for (ii = ns; ii < ne; ii++)
      {
         jj_begin_row = jj_counter;        
         jj_begin_row_offd = jj_counter_offd;
         SetNonZeroOffset(P_diag_i, ii, jj_counter);
         SetNonZeroOffset(P_offd_i, ii, jj_counter_offd);
         i = old_coarse_to_fine[ii];
         /*--------------------------------------------------------------------
         ...
         else if (CF_marker[i] == -2)
         {
            ...
            foreach_nonzero(S_diag, i, i1, jj)
            {
               ...
               if (CF_marker[i1] >= 0)
               {
                  if (P_marker[i1] < jj_begin_row)
                  {
                     InterpSetZero(P_diag_j, P_diag_data, P_marker, i1,
                                   fine_to_coarse, &jj_counter);
                  }
               }
               else  if (CF_marker[i1] != -3)
               {
                  ...
                  foreach_nonzero(S_diag, i1, k1, kk)
                  {
                     if (CF_marker[k1] >= 0)
                     {
                        if(P_marker[k1] < jj_begin_row)
                        {
                           InterpSetZero(P_diag_j, P_diag_data, P_marker, k1,
                                         fine_to_coarse, &jj_counter);
                        }
                     }
                  }
            ...
            { /* i1 is a c-point and strongly influences i, accumulate
               * a_(i,i1) into interpolation weight */
               ...
               else if(P_marker[i1] == strong_f_marker)
               {
                  ...
                  {
                     if((P_marker[i2] >= jj_begin_row || i2 == i) && IsCorrectSign(A_diag, sgn, jj1))
                  ...
                  if(sum != 0)
                  {
                     ...
                     foreach_nonzero_skipfirst(A_diag, i1, i2, jj1)
                     {
                        if(P_marker[i2] >= jj_begin_row && IsCorrectSign(A_diag, sgn, jj1))
                           DistributeIntoP(A_diag, P_diag_data, P_marker, i2, jj1, distribute);
   ...
   for (ii = 0; ii < n_coarse_old; ii++)
   {
      ...
      else if (CF_marker[i] == -2)
      {
         foreach_nonzero(S_diag, i, i1, jj)
         {
            if (CF_marker[i1] > 0)
            { /* i1 is a C point */
               OnCPointSetAndInc(P_diag_i, P_marker, &jj_counter, ii, i1);
            }
            else if (CF_marker[i1] != -3)
            { /* i1 is a F point, loop through it's strong neighbors */
               foreach_nonzero(S_diag, i1, k1, kk)
               {
                  if (CF_marker[k1] > 0)
                  {
                     OnCPointSetAndInc(P_diag_i, P_marker, &jj_counter, ii, k1);
                  } 
               }
               if(num_procs > 1)
               {
                  ...
                  * neither. */
                  foreach_nonzero(Sop, i1, k1, kk)
                  {
                     ...
                     { /* In S_diag */
                        ...
                        {
                           OnCPointSetAndInc(P_diag_i, P_marker, &jj_counter, ii, loc_col);
                        }
                     }
                     else
                     {
      ...
      {         
         ...
         foreach_nonzero(S_diag, i, i1, jj)
         {
            ...
            if (CF_marker[i1] > 0)
            {
               if (P_marker[i1] < jj_begin_row)
               {
                  InterpSetZeroNoFToC(P_diag_j, P_diag_data, P_marker, i1,
                                      &jj_counter);
               }
            }
            else  if (CF_marker[i1] != -3)
                                     ...
                                     hypre_ParCSRMatrix  **P_ptr)
{
   ...
   for (ii = 0; ii < n_coarse_old; ii++)
   {
      ...
      else if (CF_marker[i] == -2)
      {
         ...
         if (num_procs > 1)
         {
            foreach_nonzero(S_offd, i, i1, jj)
            {
               if(col_offd_S_to_A)
