...
HYPRE_Int
hypre_BoomerAMGDDTestSolve( void      *amgdd_vdata,
                   hypre_ParCSRMatrix *A,
                   hypre_ParVector    *f,
                   hypre_ParVector    *u                )
{
   ...
   HYPRE_Int num_comp_cycles = hypre_ParAMGDDDataFACNumCycles(amgdd_data);
   ...
   /* if (hypre_ParAMGDDDataUseRD(amgdd_data)) */
   if (0)
   {
   ...
   for (proc = 0; proc < num_procs; proc++)
   {
      ...
      for (level = 0; level < num_levels; level++)
      {
         ...
         if (level < num_levels) SetRelaxMarker(hypre_ParAMGDDDataCompGrid(amgdd_data)[level], relax_marker[level], proc);
   ...
   for (level = current_level; level < num_levels; level++)
   {
      ...
      if (nodes_to_add)
      {
         ...
         for (i = 0; i < total_num_nodes; i++)
         {
            ...
            {
               error_code = hypre_BoomerAMGDD_RecursivelyBuildPsiComposite(i, padding[level] + num_ghost_layers, compGrid[level], add_flag[level], 0);
               if (error_code)
               {
         ...
         if (level != num_levels-1)
         {
            ...
            hypre_BoomerAMGDD_MarkCoarse(list,
              add_flag[level+1],
...
hypre_BoomerAMGDD_TestCompGrids2(hypre_ParAMGDDData *amgdd_data)
{
   ...
   {
      HYPRE_Int level;
      for (level = hypre_ParAMGDDDataStartLevel(amgdd_data); level < num_levels; level++)
      {
         /////////////////////////////////
         // Broadcast all info
         /////////////////////////////////
