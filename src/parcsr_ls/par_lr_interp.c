			      ...
			      hypre_ParCSRMatrix  **P_ptr)
{
   ...
   for (i = 0; i < n_fine; i++)
   {
      ...
      {
         foreach_nonzero(S_diag, i, i1, jj)
         {
            if (CF_marker[i1] >= 0)
            { /* i1 is a C point */
               OnCPointSetAndInc(P_diag_i, P_marker, &jj_counter, i, i1);
            }
            else if (CF_marker[i1] != -3)
            { /* i1 is a F point, loop through it's strong neighbors */
               foreach_nonzero(S_diag, i1, k1, kk)
               {
                  if (CF_marker[k1] >= 0)
                  {
                     OnCPointSetAndInc(P_diag_i, P_marker, &jj_counter, i, k1);
                  } 
               }
               if(num_procs > 1)
               {
                  foreach_nonzero_mapped(S_offd, i1, k1, kk, col_offd_S_to_A)
                  {
                     if (CF_marker_offd[k1] >= 0)
                     {
                        OnCPointSetAndInc_WithCF(P_offd_i, P_marker_offd,
                                                 tmp_CF_marker_offd,
                                                 &jj_counter_offd, i, k1);
                     }
                  }
               }
            }
         ...
         if (num_procs > 1)
         {
            foreach_nonzero_mapped(S_offd, i, i1, jj, col_offd_S_to_A)
            {
               if (CF_marker_offd[i1] >= 0)
               {
                  OnCPointSetAndInc_WithCF(P_offd_i, P_marker_offd,
                                           tmp_CF_marker_offd,
                                           &jj_counter_offd, i, i1);
               }
                  ...
                  * neither. */
                  foreach_nonzero(Sop, i1, k1, kk)
                  {
                     ...
                     { /* In S_diag */
                        ...
                        {
                           OnCPointSetAndInc(P_diag_i, P_marker,
                                             &jj_counter, i, loc_col);
                        }
                     }
                     else
                     {
   ...
   for (i = 0; i < n_fine; i++)
   {
      ...
      if (CF_marker[i] >= 0)
      {
         InterpSetIdentNoMarker(P_diag_j, P_diag_data, i,
                                fine_to_coarse, &jj_counter);
      }
      ...
      {         
         ...
         foreach_nonzero(S_diag, i, i1, jj)
         {
            ...
            if (CF_marker[i1] >= 0)
            {
               ...
               {
                  InterpSetZeroNoFToC(P_diag_j, P_diag_data, P_marker,
                                      i1, &jj_counter);
               }
            }
            else  if (CF_marker[i1] != -3)
               ...
               if(num_procs > 1)
               {
                  foreach_nonzero_mapped(S_offd, i1, k1, kk, col_offd_S_to_A)
                  {
                     if(CF_marker_offd[k1] >= 0)
                     {
                        ...
                        {
                           InterpSetZeroNoFToC(P_offd_j, P_offd_data,
                                               P_marker_offd, k1,
                                               &jj_counter_offd);
                        }
                     }
                  ...
                  foreach_nonzero(Sop, i1, k1, kk)
                  {
                     if(k1 >= col_1 && k1 < col_n)
                     {
                        ...
                        if(CF_marker[loc_col] >= 0)
                        {
                           ...
                           {
                              InterpSetZeroNoFToC(P_diag_j, P_diag_data,
                                                  P_marker, loc_col,
                                                  &jj_counter);
                           }
                        }
         ...
         ihat[i] = cnt_f;
         ipnt[cnt_f] = i;
         InitAhatStdInterp(A_diag, i, ahat, &cnt_f);
         foreach_nonzero_skipfirst(A_diag, i, i1, jj)
         {
         ...
         if(num_procs > 1)
         {
            foreach_nonzero(A_offd, i, i1, jj)
            {
               ...
               else
               {
                  ...
                  {
                     InitDistributeStdInterp(A_ext, A_offd, i1, jj, &distribute);

                     foreach_nonzero_skipfirst(A_ext, i1, k1, kk)
                     {
                        if(k1 >= col_1 && k1 < col_n)
                        { /*diag*/
                           loc_col = k1 - col_1;
         ...
         else
         {
            ...
            InterpolateWeightStdInterpNoCheck(P_diag_j, P_diag_data,
                                              ihat, ipnt, ahat, alfa,
                                              jj_begin_row, jj_end_row,
                                              cnt_f, fine_to_coarse);
            if(num_procs > 1)
            {
               InterpolateWeightStdInterpNoCheckNoFToC(P_offd_j, P_offd_data,
                                                       ihat_offd, ipnt_offd,
                                                       ahat_offd, alfa,
                                                       jj_begin_row_offd,
                                                       jj_end_row_offd,
...
#pragma omp parallel private(i,my_thread_num,num_threads,start,stop,coarse_counter,jj_counter,jj_counter_offd, P_marker, P_marker_offd,jj,kk,i1,k1,loc_col,jj_begin_row,jj_begin_row_offd,jj_end_row,jj_end_row_offd,diagonal,sum,sgn,jj1,i2,distribute,strong_f_marker)
   ...
   { 
       ...
       * For example the first phase would compute 
      ...
      for (i = start; i < stop; i++)
      {
         SetNonZeroOffset(P_diag_i, i, jj_counter);
         if (num_procs > 1)
            SetNonZeroOffset(P_offd_i, i, jj_counter_offd);
         ...
         /*--------------------------------------------------------------------
          *  If i is an F-point, interpolation is from the C-points that
          *  strongly influence i, or C-points that strongly influence F-points
          *  that strongly influence i.
          *--------------------------------------------------------------------*/
         else if (CF_marker[i] != -3)
         {
            foreach_nonzero(S_diag, i, i1, jj)
            {
               ...
               { /* i1 is a F point, loop through its strong neighbors */
                  foreach_nonzero(S_diag, i1, k1, kk)
                  {
                     if (CF_marker[k1] >= 0)
                     {
                  ...
                  { /* F point; look at neighbors of i1. Sop contains global col
                     ...
                     * neither. */
                     ...
                     foreach_nonzero(Sop, i1, k1, kk)
                     {
                        ...
                        { /* In S_diag */
                           loc_col = k1-col_1;
                           OnCPointSetAndInc(P_diag_i, P_marker, &jj_counter,
                                             i, loc_col);
                        }
                        else
                        {
...
#pragma omp barrier
#endif
      SetNonZeroOffset(P_diag_i, stop, jj_counter);
      SetNonZeroOffset(P_offd_i, stop, jj_counter_offd);
...
#pragma omp barrier
      ...
      if(my_thread_num > 0)
      {
         ...
         {
            AccumulateNonZeroOffset(P_diag_i, i, diag_offset[my_thread_num-1]);
            AccumulateNonZeroOffset(P_offd_i, i, offd_offset[my_thread_num-1]);
         }
...
#pragma omp barrier
      ...
      if(my_thread_num == 0)
      {
         ...
         P_diag_size =  GetNonZeroOffset(P_diag_i, n_fine);
         P_offd_size = GetNonZeroOffset(P_offd_i, n_fine);
...
#pragma omp barrier
      ...
      for (i = start; i < stop; i++)
      {
         jj_begin_row = GetNonZeroOffset(P_diag_i, i);
         jj_begin_row_offd = GetNonZeroOffset(P_offd_i, i);
         ...
         else if (CF_marker[i] != -3)
         {
            ...
            foreach_nonzero(S_diag, i, i1, jj)
            {
               ...
               if (CF_marker[i1] >= 0)
               {
                  if (P_marker[i1] < jj_begin_row)
                  {
                     InterpSetZero(P_diag_j, P_diag_data, P_marker,
                                   i1, fine_to_coarse, &jj_counter);
                  }
               }
               else if (CF_marker[i1] != -3)
               {
                  ...
                  else if (CF_marker_offd[i1] != -3)
                  {
                     ...
                     foreach_nonzero(Sop, i1, k1, kk)
                     {
                        ...
                        if(k1 >= col_1 && k1 < col_n)
                        {
                           ...
                           if(P_marker[loc_col] < jj_begin_row)
                           {
                              InterpSetZero(P_diag_j, P_diag_data,
                                            P_marker, loc_col,
                                            fine_to_coarse, &jj_counter);
                           }
                        }
            ...
            InitDiagonal(A_diag, i, &diagonal);
            ...
            { /* i1 is a c-point and strongly influences i, accumulate
              * a_(i,i1) into interpolation weight */
               if (P_marker[i1] >= jj_begin_row)
               {
                  AccumulateIntoP(A_diag, P_diag_data, P_marker, i1, jj);
               }
               else if(P_marker[i1] == strong_f_marker)
               {
                  sum = zero;
                  InitSgn(A_diag, i1, &sgn);

                  /* Loop over row of A for point i1 and calculate the sum
                   * of the connections to c-points that strongly influence i. */
                  foreach_nonzero_skipfirst(A_diag, i1, i2, jj1)
                  {
                     if((P_marker[i2] >= jj_begin_row || i2 == i) &&
                        IsCorrectSign(A_diag, sgn, jj1))
                        AccumulateConnection(A_diag, jj1, &sum);
                  }
                  if(num_procs > 1)
                  {
                     foreach_nonzero(A_offd, i1, i2, jj1)
                     {
                        if(P_marker_offd[i2] >= jj_begin_row_offd &&
                           IsCorrectSign(A_offd, sgn, jj1))
                           AccumulateConnection(A_offd, jj1, &sum);
                     }
                  }
                  if(sum != 0)
                  {
                     ...
                     foreach_nonzero_skipfirst(A_diag, i1, i2, jj1)
                     {
                        if(P_marker[i2] >= jj_begin_row &&
                           IsCorrectSign(A_diag, sgn, jj1))
                           DistributeIntoP(A_diag, P_diag_data, P_marker, i2, jj1,
                                           distribute);
                        if(i2 == i && IsCorrectSign(A_diag, sgn, jj1))
                           DistributeConnection(A_diag, jj1, distribute, &diagonal);
                     }
                     if(num_procs > 1)
                     {
                        foreach_nonzero(A_offd, i1, i2, jj1)
                        {
                           if(P_marker_offd[i2] >= jj_begin_row_offd &&
                              IsCorrectSign(A_offd, sgn, jj1))
                              DistributeIntoP(A_offd, P_offd_data,
                                              P_marker_offd, i2, jj1,
                                              distribute);
                        }
                     }
                  }
                  ...
                  {
                     AccumulateConnection(A_diag, jj, &diagonal);
                  }
               }
            ...
            if(num_procs > 1)
            {
               ...
               {
                  if(P_marker_offd[i1] >= jj_begin_row_offd)
                     AccumulateIntoP(A_offd, P_offd_data, P_marker_offd,
                                     i1, jj);
                  else if(P_marker_offd[i1] == strong_f_marker)
                  {
                     sum = zero;
                     foreach_nonzero(A_ext, i1, k1, jj1)
                     {
                        if(k1 >= col_1 && k1 < col_n)
                        { /* diag */
                           loc_col = k1 - col_1;
                           if(P_marker[loc_col] >= jj_begin_row || loc_col == i)
                              AccumulateConnection(A_ext, jj1, &sum);
                        }
                        else
                        { 
                           loc_col = -k1 - 1;
                     ...
                     if(sum != 0)
                     {
                        ...
                        foreach_nonzero(A_ext, i1, k1, jj1)
                        {
                           ...
                           { /* diag */
                              loc_col = k1 - col_1;
                              if(P_marker[loc_col] >= jj_begin_row)
                              DistributeIntoP(A_ext, P_diag_data,
                                              P_marker, loc_col, jj1,
                                              distribute);
                              if(loc_col == i)
                                 DistributeConnection(A_ext, jj1, distribute,
                                                      &diagonal);
                           }
                           else
                           { 
                              loc_col = -k1 - 1;
                              if(P_marker_offd[loc_col] >= jj_begin_row_offd)
                                 DistributeIntoP(A_ext, P_offd_data,
                                                 P_marker_offd, loc_col, jj1,
                                                 distribute);
                           }
                        }
                     }
                     ...
                     {
                        AccumulateConnection(A_offd, jj, &diagonal);
                     }
                  }
                  else if (CF_marker_offd[i1] != -3)
                  {
            ...
            if (diagonal)
            {
               ScaleRowByDiagonal(jj_begin_row, jj_end_row,
                                  P_diag_data, diagonal);
               ScaleRowByDiagonal(jj_begin_row_offd, jj_end_row_offd,
                                  P_offd_data, diagonal);
            }
         }
         strong_f_marker--;
      }
         ...
         if(num_procs > 1)
         {
            foreach_nonzero_mapped(S_offd, i, i1, jj, col_offd_S_to_A)
            {
               ...
               { /* i1 is a C point direct neighbor */
                  CF_marker_offd[i1] = 2;
                  OnCPointSetAndInc_WithCF(P_offd_i, P_marker_offd, tmp_CF_marker_offd,
                                           &jj_counter_offd, i, i1);
               }
            }
                  ...
                  * neither. */
                  ...
                  if(!common_c)
                  {
                     foreach_nonzero(Sop, i1, k1, kk)
                     {
                        ...
                        { /* In S_diag */
                           loc_col = k1-col_1;
                           OnCPointSetAndInc(P_diag_i, P_marker, &jj_counter, i, loc_col);
                        }
                        else
                        { 
                           loc_col = -k1 - 1;
      ...
      else if (CF_marker[i] != -3)
      {
         ...
         if ( num_procs > 1)
         {
            foreach_nonzero_mapped(S_offd, i, i1, jj, col_offd_S_to_A)
            {
               if ( CF_marker_offd[i1] > 0)
               {
                  ...
                  if(P_marker_offd[i1] < jj_begin_row_offd)
                  {
                     InterpSetZeroNoFToC(P_offd_j, P_offd_data, P_marker_offd,
                                         i1, &jj_counter_offd);
        ...
        foreach_nonzero(S_diag, i, i1, jj)
        {
         ...
         foreach_nonzero_skipfirst(A_diag, i, i1, jj)
         {
            ...
            else if(P_marker[i1] == strong_f_marker)
            {
               ...
               if(sum != 0)
               {
                  ...
                  /* Loop over row of A for point i1 and do the distribution */
                  foreach_nonzero(A_diag, i1, i2, jj1)
                  {
                     if(P_marker[i2] >= jj_begin_row &&
                        IsCorrectSign(A_diag, sgn, jj1))
                        DistributeIntoP(A_diag, P_diag_data, P_marker, i2, jj1,
                  ...
                  if(num_procs > 1)
                  {
                     foreach_nonzero(A_offd, i1, i2, jj1)
                     {
                        if(P_marker_offd[i2] >= jj_begin_row_offd &&                                   
                           IsCorrectSign(A_offd, sgn, jj1))
                           DistributeIntoP(A_offd, P_offd_data, P_marker_offd,
                                           i2, jj1, distribute);
                     }
                  }
               }
         ...
         if(num_procs > 1)
         {
            ...
            {
               if(P_marker_offd[i1] >= jj_begin_row_offd)
                  AccumulateIntoP(A_offd, P_offd_data, P_marker_offd, i1, jj);
               else if(P_marker_offd[i1] == strong_f_marker)
               {
                  sum = zero;
                  sgn = 1;
                  ...
                  if(sum != 0)
                  {
                     ...
                     foreach_nonzero(A_ext, i1, k1, jj1)
                     {
                        ...
                        { /* diag */
                           loc_col = k1 - col_1;
                           if(P_marker[loc_col] >= jj_begin_row)
                              DistributeIntoP(A_ext, P_diag_data, P_marker,
                                              loc_col, jj1, distribute);
                           if(loc_col == i)
                              DistributeConnection(A_ext, jj1, distribute, &diagonal);
                        }
                        else
                        { 
                           loc_col = -k1 - 1;
                           if(P_marker_offd[loc_col] >= jj_begin_row_offd)
                              DistributeIntoP(A_ext, P_offd_data, P_marker_offd,
                                              loc_col, jj1, distribute);
                        }
                     }
                  }
            ...
            else if(P_marker[i1] == strong_f_marker)
            {
               ...
               if(sum != 0)
               {
                  ...
                  foreach_nonzero_skipfirst(A_diag, i1, i2, jj1)
                  {
                     if(P_marker[i2] >= jj_begin_row &&
                        IsCorrectSign(A_diag, sgn, jj1))
                        DistributeIntoP(A_diag, P_diag_data, P_marker,
                                        i2, jj1, distribute);
                  }
                  if(num_procs > 1)
                  {
