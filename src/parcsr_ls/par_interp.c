...
GetGhostRows(hypre_ParCSRMatrix *A, hypre_CSRMatrix **A_ext_ptr,
             HYPRE_Int **A_ext_i_ptr, HYPRE_Int **A_ext_j_ptr,
             HYPRE_Real **A_ext_data_ptr, HYPRE_Int *col_map_offd,
             HYPRE_Int num_procs, HYPRE_Int num_cols_A_offd, HYPRE_Int col_1,
             HYPRE_Int col_n)
{
   HYPRE_Int index, i, j, k, kc;
   hypre_CSRMatrix *A_ext;
   HYPRE_Int *A_ext_i, *A_ext_j;
   HYPRE_Real *A_ext_data;

   if (num_procs > 1)
   {
      *A_ext_ptr      = hypre_ParCSRMatrixExtractBExt(A,A,1);
      A_ext = *A_ext_ptr;
      *A_ext_i_ptr    = hypre_CSRMatrixI(A_ext);
      *A_ext_j_ptr    = hypre_CSRMatrixJ(A_ext);
      *A_ext_data_ptr = hypre_CSRMatrixData(A_ext);
      A_ext_i = *A_ext_i_ptr;
      A_ext_j = *A_ext_j_ptr;
      A_ext_data = *A_ext_data_ptr;
   }

   index = 0;
   for (i=0; i < num_cols_A_offd; i++)
   {
      foreach_nonzero(A_ext, i, k, j)
      {
         if (k >= col_1 && k < col_n)
         {
...
BuildInterpZeroDiagonal(HYPRE_Int begin_row, HYPRE_Int end_row,
                            ...
                            hypre_ParCSRMatrix  **P_ptr)
{
   ...
   GetGhostRows(A, &A_ext, &A_ext_i, &A_ext_j, &A_ext_data,
                col_map_offd, num_procs, num_cols_A_offd, col_1, col_n);
...
#pragma omp parallel for private(i,j,jl,i1,i2,jj,jj1,ns,ne,size,rest,sum,diagonal,distribute,P_marker,P_marker_offd,strong_f_marker,jj_counter,jj_counter_offd,sgn,c_num,jj_begin_row,jj_end_row,jj_begin_row_offd,jj_end_row_offd) HYPRE_SMP_SCHEDULE
   ...
   for (jl = 0; jl < num_threads; jl++)
   {
      ...
      for (i = ns; i < ne; i++)
      {
         ...
         if (CF_marker[i] >= 0)
         {
            InterpSetIdentSetOffset(P_diag_i, P_diag_j, P_diag_data,
                                    i, fine_to_coarse, &jj_counter);
         }
         ...
         {         
            ...
            if (num_procs > 1)
            {
               foreach_nonzero(A_offd, i, i1, jj)
               {
                  ...
                  else if (P_marker_offd[i1] == strong_f_marker)
                  {
                     ...
                     /* find row number */
                     c_num = GetColumn(A_offd, jj);

                     InitSgn(A_ext, c_num, &sgn);
                     foreach_nonzero(A_ext, c_num, i2, jj1)
                     {
            ...
            if (diagonal == 0.0)
            {
               ...
               BuildInterpZeroDiagonal(jj_begin_row, jj_end_row,
                                       P_diag_data);
               BuildInterpZeroDiagonal(jj_begin_row_offd, jj_end_row_offd,
                                       P_offd_data);
            }
            else
            {
         ...
         SetNonZeroOffset(P_offd_i, i+1, jj_counter_offd);
      }
   ...
   CompressP(A, P, P_diag, P_diag_i, P_diag_j, P_diag_data, P_diag_size,
             P_offd, P_offd_i, P_offd_j, P_offd_data, P_offd_size,
             trunc_factor, max_elmts, CF_marker, num_cols_A_offd, n_fine,
             fine_to_coarse_offd);
